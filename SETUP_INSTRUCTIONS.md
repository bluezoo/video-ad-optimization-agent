...
- **Same workstream (14c), owner-directed model bump:** `AGENT_MODEL` default bumped from `gemini-3.6-flash` to `gemini-3.7-flash` (`app/config.py`) — routing/instruction behavior unaffected (same GA/global-region contract as the Phase 14a/14b bump above), only the model id changed. Override via `AGENT_MODEL` in `app/.env` to pin a different version.
