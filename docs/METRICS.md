...
| `sensor_dwell` | Distribution of visit-duration bins per 15-minute slot, **plus undocumented `distribution_average_duration` / `distribution_median_duration` scalars** (live scan, 2026-07-25) | Map to our scalar `dwell_time_seconds` by reading BlueZoo's own average/median, not by weighting bins — units unverified (zero-row account); Phase 11 confirms against real rows. |
