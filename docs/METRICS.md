...
| `sensor_visits` `valid` column | Per-**sensor** one-way commissioning-acceptance flag ("accepted into service after verified reporting" — verified against live data 2026-07-27: one-way `false`→`true` flips, ~0.99 pulse-health for valid vs ~0.00 for invalid, no flip-back on later degradation); `null` = rows predating the column (2021-10-14), not a third state | **Recommended rule R (pending BlueZoo confirmation): count impressions only from `valid IS TRUE` rows (`WHERE valid`), with the exclusion explicit and logged.** Go-forward impact ~1.6× (the historical ~4× figure was inflated by a retired fleet). Evidence: `.docs/version2-plan/working-docs/bluezoo-semantics-resolution/findings.md`; see open question 18. |
