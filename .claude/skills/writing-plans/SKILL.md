...
**Context:** If working in an isolated worktree, it should have been created via `starting-a-workstream` (this repo's workstream kickoff skill) at execution time, on a `version_2_<feature>` branch off `version_2`.

**Save plans to:**
- Inside a `.docs/version2-plan/` workstream: `.docs/version2-plan/working-docs/<NN>-<name>/plan.md`, co-located with that workstream's working doc. Seed this plan from the approved working doc, not a freeform spec — the working doc's "Implementation approach" section is the spec.
- Otherwise (work outside the version2-plan lifecycle): `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
## Plan Approval (workstream checkpoint 3)

Inside a `.docs/version2-plan/` workstream, the saved plan needs the owner's explicit approval before execution — same explicit-yes standard as the working doc. On approval (per `tracking-workstream-progress`):

- Append checkpoint 3 ("plan approved") to the workstream's `WORK_LOG.md`.
- Update the workstream's `STATUS.md` row to `implement in progress` (single-writer, in the main checkout).
