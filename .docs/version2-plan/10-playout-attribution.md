...
> 1. **Fixture data comes from Phase 4's ported deterministic generator, not hand-shaping.** The generator emits BlueZoo-shaped 15-minute visit frames for any `(campaign, screen, date)` key on demand, so "a handful of hand-shaped fixture records" (step 4 below) becomes "pick a handful of keys" — same test-scale intent, no fixture authoring, and it automatically covers products/campaigns created later (Phase 14).
> 2. **Adopt the BlueZoo-shaped table set as the internal demo schema** (owner's one-system decision: demo mimics BlueZoo's schema/API so the live connector drops in behind the same seam). The donor repo's five-table shape — visits at 15-min grain, dwell distributions, unique-visitor buckets, flow transitions, and a `video_attribution` window table — is the working model; validate the mimic against BlueZoo's actual schema docs when the client provides them (see `99-open-questions.md` Q2). The current per-video daily `video_metrics` table becomes a derived aggregation during this phase (Phase 4's derivation layer is replaced by this phase's real join), not the source of truth. Cascade note: `docs/METRICS.md` (Phase 2) definitions stay valid — they define metric *semantics*, not storage grain — but re-check its table references when this lands.
