...
- **Demo mode stays first-class.** Nothing in this plan makes the demo worse, slower, or dependent on live (BlueZoo/PoS) credentials. `APP_MODE=demo` (added in Phase 5) must remain the default. **Caveat found during Codex review, not yet resolved:** demo mode today is not actually zero-external-account — `app/storage.py:189 (also 208, 224)` requires a real GCS bucket/client for product images and raises without one, and the primary video pipeline (`app/tools/video_tools.py:520`) reads product images through that path. Either narrow this philosophy promise to "no BlueZoo/PoS credentials required" or add a local-fixture asset provider before Phase 10's demo-mode wiring can honestly claim "zero external accounts" — tracked as open question 16.
...
| 9 | `10-playout-attribution.md` | Demo-only ad-play records (client's exact `AdPlayRecord` schema) + per-ad-play impressions/revenue join from deterministic fixtures | Medium | Phases 3, 4, 7 |
