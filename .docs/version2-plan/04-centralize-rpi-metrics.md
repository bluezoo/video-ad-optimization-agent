# Phase 3 — Centralize RPI/Metric Computation

## Goal

Replace the independent, ad-hoc metric computations scattered across the codebase (a dozen-plus sites, precise inventory below) with calls to one shared function per metric, built on the definitions from Phase 2. This is what makes the Phase 1 weekly-RPI bug (and any similar bug) structurally impossible to reintroduce elsewhere.
...
Phase 2 (`docs/METRICS.md` definitions must exist first).
