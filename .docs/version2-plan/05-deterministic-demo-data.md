...
5. Update all three call sites (`app/database/mock_data.py:383`, `app/tools/review_tools.py:168`, `app/tools/review_tools.py:563`) to use the single new function.
...
### Forward compatibility (Phase 9)

Keep this a narrow aggregate generator, and factor the revenue side so Phase 9 (`10-playout-attribution.md`) can later re-key revenue by `(product_id, time-window)` without rework. Phase 9 will reroute this generator to derive impressions/revenue via an ad-play join, so don't bake per-video-per-day assumptions any deeper than necessary — isolate the revenue computation so the (product_id, window) keying can be swapped in later. This mirrors what `10-playout-attribution.md:53` already demands of this phase.
...
- [ ] `generate_additional_metrics()` (`review_tools.py:563`) also uses the unified generator — a test confirms this third call site wasn't missed.
