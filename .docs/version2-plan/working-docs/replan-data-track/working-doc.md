...
**Trigger:** owner's strategic pivot + sibling-repo findings (`.docs/.context/2026-07-16-sibling-repo-reassessment.md`, local). Enforced trigger from that review: Phase 5 may not start until this replan merges.
...
- The donor is a **donor, not a transplant**: dormant since May, unpushed dirty flagship branch, missing Phases 1/2 fixes, fashion-hardcoded, GCP-billed-project default.
...
4. **Phase 5 fate (owner delegated; call made):** rewrite as "port the donor generator, conformed to BlueZoo shape; retire both existing mock generators." Prime motive stated by owner: general agent + closest possible alignment with BlueZoo data schema and metrics.

## Implementation approach (the amendments)

1. `05-deterministic-demo-data.md` (Phase 5) — rewrite around porting the donor's `seed.py`.
2. `06-app-mode-config-skeleton.md` (Phase 6) — `APP_MODE` resolves to an internal provider name (one config axis); un-protect import-time `populate_mock_data()` (seeding becomes explicit, idempotent, gated).
3. `08-product-schema-generalization.md` (Phase 8) — flip step 6: product CRUD is real, lives in the new onboarding phase.
4. `10-playout-attribution.md` (Phase 10) — fixtures come from the ported generator; adopt the donor's dual-write bridge + attribution-join pattern; adopt the BlueZoo 5-table shape.
5. `11-live-bluezoo-adapter.md` (Phase 11) — split 11a (port the seam now — unblocked) / 11b (live conformer — blocked on BlueZoo schema/API docs).
6. New `15-product-onboarding.md` (Phase 15) — tools-first onboarding + local storage write path + kill personal-bucket default + preseeded fashion becomes a selectable dataset.
...
- Phases 12 (live PoS), 13, 14a/14b docs — untouched beyond dependency-line touch-ups if strictly required.
