...
- **DIVERGENCE — the "Phase 1 weekly-RPI regression test" the Validation section says to re-run does not exist.** `grep -rni "week" tests/` returns nothing; ws02 only flagged the bug with the comment. This workstream must *write* that regression test, not re-run it. Phase doc to be amended with provenance.
...
- Unifying the two `_generate_mock_video_metrics()` into one deterministic generator — Phase 4 (`05-deterministic-demo-data.md`).
...
- BlueZoo/PoS integration (Phases 10-11); chart redesign (Phase 6).
