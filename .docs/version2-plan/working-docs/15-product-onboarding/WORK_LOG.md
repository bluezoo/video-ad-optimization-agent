...
## 2026-07-22 — checkpoint 5 (round 2): from-scratch-onboarding 5/5 PASS
Re-run against 8544fe8, fresh DB, DEMO_DATASET=none + GCS_BUCKET unset, one session (f0a5b7a2). Scenes 1.1/2.1/2.2/2.3/3.1 all PASS: empty catalog -> create_product (campaign_agent, image_status pending) -> generate_product_image (real gemini-3-pro-image call, artifact rendered inline, image_status available, no image_url) -> browse-ready via list_products -> create_campaign ("Aurora Cold Brew 330Ml - Demo Store"). Global condition PASS: zero storage.googleapis.com matches across all session JSON, including the scene that ran the live image pipeline. Evidence: /tmp/ws15-verify-fromscratch/ (scene*.json, full_session.json, server log). Fashion F1+F3 regression next.
## 2026-07-23 — checkpoint 5 complete: demo-scenario verification PASS (both scenarios)
- from-scratch-onboarding.md: 5/5 PASS (round 2, after routing fix 8544fe8) — see checkpoint-5 round-2 entry above.
