...
Note: `make lint` fails with 174 pre-existing ruff errors across untouched files (repo-wide condition, not introduced here — candidate for Phase 2 cleanup); the new smoke script itself lints clean.
...
Blast radius: Phase 2 doc amended (new item 8, marked already-fixed, provenance note). No other phase doc references conftest paths.
...
Suites: test-unit 82 passed/1 skipped; test-integration 5 passed (live, now on gemini-3.5-flash, with google-adk[eval] installed); test-e2e 22 passed/3 pre-existing failures (see DISCOVERY above, owned by Phase 2 item 9).
...
Verifier also observed the agent NARRATING "Gemini 2.0 Flash" in chat text while actually calling gemini-3-pro-image — that's the known stale MEDIA_AGENT_INSTRUCTION text at agent.py:200, already owned by Phase 2 item 6; no action here.

## 2026-07-14 20:00 — branch code review: READY TO MERGE
Whole-branch review (general-purpose reviewer, version_2..HEAD): zero Critical/Important issues, 4 Minor. Reviewer independently reproduced test results and validation greps. Minor fixes applied same session: test_config teardown now uses monkeypatch.undo() before reload (env-sync correctness); smoke script's pre-rename getattr shim removed (dead post-rename); scenario F1's campaign-count assertion softened (tolerate leftover local campaigns); phase-doc validation boxes ticked with evidence pointer (last box deliberately left open — closes with Phase 2's share of the Gemini 2.0 cleanup). Re-verified after fixes: unit 82 passed/1 skipped, ruff clean, Stage-1 smoke OK.

## 2026-07-14 20:40 — owner-requested pre-merge probe: preview models via the new env overrides
Owner asked to test Nano Banana flash lite and Omni before merging.
**Nano Banana 2 Lite (`gemini-3.1-flash-lite-image`): WORKS as a drop-in.** `IMAGE_GENERATION_MODEL` override through the unmodified `generate_scene_image()` path produced an on-brief image (sage satin camisole, studio, elegant; evidence/preview-probe-nano-banana-lite-scene.png). Output 102,601 bytes vs ~1.2-1.5 MB from the GA default — consistent with its 1K-output-token cap. Phase 14a validation item 1 pre-verified; amended 14a doc with provenance.
**Gemini Omni Flash (`gemini-omni-flash-preview`): NOT a drop-in — as the plan predicted.** (a) 404 NOT_FOUND on the `generate_videos` surface (Vertex, global). (b) `google-genai` 2.11.0 does expose `client.interactions` — answers 14b's unverified-SDK question for ≥2.11.0. (c) Bounded prototype attempts at `interactions.create` (raw request body, text_to_video and image_to_video, field names matching the SDK's own TypedDicts) all fail with 400 "value 'UNKNOWN' is not supported for 'type'" — raw dicts aren't serialized as the wire expects; 14b's step-3 prototype must use the SDK's typed request path. Amended 14b doc with provenance. Veo 3.1 GA stays the default and the only working video path, exactly per plan.

## 2026-07-14 21:10 — FINISH: merged (checkpoint 6)
PR #1 (https://github.com/bluezoo/video-ad-optimization-agent/pull/1) merged into version_2, merge commit 9f12738. Remote and local feature branches deleted; worktree removed. STATUS.md row → merged. Phase 1 complete: all model config on GA defaults with env overrides, release gate + scenario F1 evidenced, three discoveries handed to Phase 2 (items 7-9), preview-model probe findings handed to Phases 14a/14b. Next workstream branches from this version_2.
