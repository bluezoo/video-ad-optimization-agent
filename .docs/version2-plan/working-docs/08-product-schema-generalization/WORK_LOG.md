...
## 2026-07-20 — post-verification note (F5 verifier observation, evidence for Phase 9)
During F5 the agent auto-chained a video generation for the beverage product; the resulting Veo prompt still carries fashion-specific text ("fashion photography", "garment", "fabric flow") — exactly the hardcoded prompt assumptions Phase 9 exists to remove (its doc already receives the retail core set as e2e input via this workstream's amendment). No F5 criteria affected; campaign description/category were clean. Logged so Phase 9's kickoff has a concrete reproduced example.

## 2026-07-19 — DISCOVERY: owner manual test surfaced prompt + product-image architecture gaps
Owner ran the pre-merge user-journey guide and hit two issues generating a video for `aurora-cold-brew-330ml`:
1. **Fashion prompt applied to beverage (expected, Phase 9):** `build_scene_image_prompt` falls back `style → category`, so `garment_desc` became "beverage" and the prompt read "…wearing a stunning beverage… The garment is clearly visible with all its details: <cold brew description>". Veo dutifully rendered a model wearing a dress made of cold-brew cans. Known limitation, but the owner now wants Phase 9 to consider a **per-vertical templating system** (and/or LLM prompt-writer), not only the planned `presentation_mode` branch.
2. **Product images are GCS-only and existence is never checked (new facts):**
   - `app/storage.py` "Product Image Storage Functions (GCS only - no local fallback)" — product images were ALWAYS GCS-only in the base repo; no workstream removed GCS (owner's recollection was of ADK's local session/artifact storage, which is unrelated). In local mode (`GCS_BUCKET` unset) product-image reads raise RuntimeError — an asymmetry vs seed-image/video storage, which does have a local path.
