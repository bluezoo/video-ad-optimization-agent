...
## Task 15 addendum: grade-layer fit verdict (2026-07-23)

**What was built:** `scripts/eval_grade_report.py` converts a `record_actuals.py` `record_to` JSON dump (Task 5/6 output — `eval_id`, `query`, `inference_ok`, `actual_tool_calls`, `final_response_text`, per-dimension harness verdicts) into a `vertexai._genai.types.common.EvaluationDataset` trace file (built with the real SDK models, not hand-rolled dicts, for schema fidelity — round-tripped through `EvaluationDataset.model_validate_json` to confirm), writes a minimal `metrics_to_run: [tool_use_quality, final_response_quality]` config, and invokes `agents-cli eval grade --traces --output --config` in the confirmed project-free mode (§5: all three flags supplied, no manifest/pyproject needed). `make test-live-report` chains `record_actuals.py` (default eval set: `review_agent.test.json`, 3 cases) into the conversion+grade script; `SKIP_RECORD=1` + `RECORD_JSON=<path>` reuses an existing recorder dump instead of re-running live inference. Output lands in `artifacts/grade_results/` (`artifacts/` gitignored).

**Live run performed (real Vertex AI Eval Service calls, 2026-07-23, `review_agent.test.json`, 3 cases, all `inference_ok`):**

| Case | Harness `tools` | Harness `answer` | agents-cli `tool_use_quality_v1` (no `agent_data.agents`) | agents-cli `tool_use_quality_v1` (with declarations) | agents-cli `final_response_quality_v1` |
|---|---|---|---|---|---|
| show-review-table | pass (1.0) | pass (1.0) | 0.0 | **1.0** | 0.0 |
| get-activation-summary | pass (1.0) | pass (1.0) | 0.33 | **1.0** | 0.0 |
| list-pending-videos | pass (1.0) | pass (1.0) | 0.0 | **1.0** | 0.6 |

**Root cause of the initial disagreement (fixed):** the first live run (`agent_data.agents` omitted) produced near-zero `tool_use_quality_v1` scores that flatly contradicted the harness's all-pass verdicts. Reading the graded rubric verdicts (`results_*.json` → `rubric_verdicts[].reasoning`) showed the server-side adaptive-rubric generator, given a trace with no declared tool schemas, assumed the agent under test *has no tools* and auto-generated rubrics like `TECHNICAL_CORRECTNESS:NO_TOOL_CALL` ("the developer instructions explicitly show that no tools were provided") — then dinged every observed `function_call` as a violation of that assumption. **Fix:** `scripts/eval_grade_report.py` now introspects the live agent graph (`app.agent.root_agent` + `sub_agents`) via the SDK's own `AgentConfig.from_agent()` (the same helper agents-cli's local inference runner uses, which wraps plain callables in ADK's `FunctionTool` to strip injected params like `tool_context`) and attaches the resulting `agent_data.agents` map to every case. Re-running the *same* recorded traces (`SKIP_RECORD=1`, no new inference) with declarations attached flipped `tool_use_quality_v1` to **1.0 on all 3 cases — full agreement with the harness's `tools`/`trajectory` dimensions.**

**Remaining, structural disagreement (not fixed, out of scope):** `final_response_quality_v1` stayed low (0.0–0.6 vs. the harness's pass=1.0 on all 3) even with declarations attached. Reading its rubric verdicts pinpoints why: the metric's adaptive rubrics ground the final answer against the tool's *actual returned data* (e.g. "does the answer present the markdown table `get_video_review_table()` returned?"), but `record_actuals.py`'s `_extract_actuals()` (`tests/integration/eval_harness.py`) only records tool call **names and args**, never the **tool response/result** — so every converted case shows the judge a tool call with no `function_response` event, and the rubric verdicts explicitly say things like *"the tool call `get_video_review_table()` returned `None`"* and then fail the agent for not matching that (fabricated) null result. This is a genuine format/data gap, not a bug in the conversion: closing it would require `eval_harness.py` to additionally capture and persist each tool's return value, which Task 15's constraints explicitly forbid touching. `final_response_match_v2` (our harness's own answer-dimension judge) doesn't have this problem because it runs directly against the live `InferenceResult`, which does carry tool-response content ADK-side — the gap is specific to the trace-export/re-grade round trip, not to the harness itself.

**Fit verdict: KEEP as an INFORMATIONAL reporting layer. Do not promote to a gate; do not drop.**

- Evidence for *not dropping*: `tool_use_quality_v1` is fully usable today and agrees with the harness's `tools`/`trajectory` verdicts 3/3 once tool declarations are attached (a fix now built into the conversion script, not a manual step) — this alone is genuine independent-judge signal worth having as a second opinion on tool selection/sequencing.
- Evidence for *not promoting to a gate*: `final_response_quality_v1` is presently unreliable on this repo's recorded traces for a structural reason (no tool-response capture) that Task 15 cannot fix without changing `eval_harness.py`'s recording contract (out of scope; would be a Task-6/recorder-format change, not a Task-15 reporting-layer change). Gating on a metric known to be systematically miscalibrated against ground truth would be worse than not gating at all. A 3-case single eval set is also too small a sample to justify a promotion decision regardless.
- If the owner wants to revisit this: the concrete next step to make `final_response_quality_v1` trustworthy is to extend the recorder (Task 5/6 territory, a separate task) to persist each tool call's return value alongside its name/args, then thread it into `eval_grade_report.py`'s `AgentEvent` construction as `function_response` parts before the final text event.
