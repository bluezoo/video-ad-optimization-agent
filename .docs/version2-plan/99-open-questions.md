...
>
> **Fully resolved against real data (same workstream, 2026-07-27).** The column is not vestigial and we now know what it holds: **9,480 of 10,075 rows (94%) carry both `campaign_id` and `campaign_name`**, across 13 campaigns, effectively **1:1 with `group_id`** (a campaign's name is the human-readable form of its group's slug) and carrying `target_uv` plus `actual_accuracy`. It is a **BlueZoo unique-visitor *measurement* campaign over a sensor group — not an advertising campaign.** That confirms the caveat below rather than undermining it, and makes the `ad_campaign_id` rename unambiguous. Two practical notes for the port: campaign names are free text and need trimming (one sampled name carried a trailing space), and `cuv` is a FLOAT extrapolation (82.81, 959.46), never an integer count. Worth one confirmation from BlueZoo that this column will never carry an advertiser's campaign id.
>
...
>
> **Confirmed with real contents (same workstream, 2026-07-27).** On MO_92 the table holds **75 groups / 93 sensors / 1,181 rows**, 2021-08-02 → 2025-12-02. Many rows share one identical timestamp, so it is **snapshot/revision-versioned**: each membership change writes a whole new set of rows stamped with that revision time (~12.7 revisions per sensor). Groups are human-named and correspond to venue-level sensor sets (here: individual rooms within a property). **Read it as-of a date (`max(timestamp) <= D`)** — never as a static lookup, and never assume the newest revision applied to historical traffic.
>
...
> **ANSWERED — closed (workstream bluezoo-live-verification, 2026-07-27).** BlueZoo granted a second account with **real data**:, cluster/account **Morpheus / MO_92**, their **staging** environment at `https://hermes.morpheus.bluezoo.io/v2/dwh`, with **its own AccessKey** (the Apollo key is rejected there — credentials are cluster-scoped, not user-scoped). 5.1M rows in `sensor_visits`, 101 sensors, 2021-03 → current, spanning multiple US and non-US timezones. Entitlements are identical to AP_599's 19 tables. So the answer to "sandbox or production?" is **a staging tenant carrying a real customer's data** — which also means MO_92 credentials and any extract are customer-confidential. The zero-data problem below is resolved; what replaced it is a **quota** problem (see Q18). Full record: `working-docs/bluezoo-live-verification/findings.md` Part 5.
>
> **Partly answered, and it moved the problem (workstream bluezoo-live-verification, 2026-07-25) — superseded by the entry above, kept for the record.** The owner obtained a real account — org "Walmart Demo", cluster/account Apollo / AP_599, Super Admin — and it authenticates and answers queries fine. So *access* is no longer the blocker. **Data is.** Every one of the 19 entitled tables returns zero rows across a 2020–2030 window: a fresh tenant with no deployed sensors. That is enough to verify structure, entitlements, query mechanics, error semantics and auth — and not enough to verify a single value. Phase 11b's `CachedBlueZooAudienceDataSource`, whose entire purpose is proving real and synthetic payloads are interchangeable, **cannot be built from this account.**
>
...
> **Weakened by real data (same workstream, 2026-07-27): fallback (b) is entitled but DORMANT.** On MO_92, `sensor_visitors_per_minute` holds 2.8M historical rows but **zero in all of 2026** — the feed is switched on as an entitlement and is not producing. So (b) cannot currently be validated for accuracy even where the table exists, and BlueZoo's answer is still needed on whether the feed is generally available, opt-in, or being retired. General lesson now carried into Phase 11b's capability probing: **entitlement ≠ population** — `list_tables` naming a table does not mean data flows, so a probe must confirm recent rows.
>
> **Fallback (b) is available on our tenant (workstream bluezoo-live-verification, 2026-07-25) — narrowed by the entry above.** `sensor_visitors_per_minute` **is entitled** on Apollo / AP_599 — no BlueZoo request needed to try it here. Its live columns are `visitors_inner` / `visitors_outer`, confirming the docs-verification's read that it is **occupancy, not visits**; using it as an impressions proxy remains an approximation with different semantics, not a like-for-like substitute. Two parts of this question survive: whether the table is generally available to *other* customers or opt-in per account (a real BlueZoo question, and the reason Phase 11b must detect entitlements via `list_tables` rather than assume them), and whether (c) — an undocumented raw-event or finer-grain path — exists at all. And with zero rows on the account, "available" means the table can be queried, not that its accuracy as a proxy has been tested against anything.
...
>
> **Mostly CLOSED against real data (same workstream, 2026-07-27, tenant MO_92).** With Q12 resolved, four of the five remaining bullets are answered empirically. Record: `findings.md` Part 5.
>
> - **Dwell bin scale — ANSWERED: 0–100 percentages**, not 0–1 shares (one slot's first three bins sum to 62.9). **`distribution_weight` — ANSWERED:** a sample/confidence weight; when it is `0.0`, all bins are `0` **and both duration scalars are NULL** while `total_visits` stays non-zero, so null dwell is a legitimate value a conformer must carry. `distribution_average_duration`/`_median_duration` are **integer seconds** — a 1:1 map onto our `dwell_time_seconds`. `docs/METRICS.md` updated.
> - **Row caps / rate limits — ANSWERED, and the premise was wrong: there is no row cap.** `run_query` is metered against a **monthly BYTES-SCANNED allowance** (BigQuery style), undocumented — ~735 MB of full-history aggregates exhausted the 1 GB/sensor-location default, after which *every* query failed, including single-day ones. Metadata and the Real-time API stayed up. There is **no endpoint to check remaining allowance.** `sensor_dwell` alone is 1,013 bytes/row, so a full `select *` is ~5.2 GB in one statement.
>   **Sized and de-escalated 2026-07-27 (BlueZoo, in writing):** the quota is an **abuse guard, not a billing meter**; it is **per-tenant configuration they raise on request**; MO_92 is now at **500 GB per sensor location** for the month, 500× the default, with explicit guidance not to worry about it. So it constrains Phase 11b's access pattern as *engineering discipline* (name your columns, narrow your windows, cost the pattern) rather than as a capacity ceiling — but exhaustion remains a distinct, non-retryable operational state the connector must surface by name, precisely so an operator knows to ask for a raise.
> - **`valid=false` exclusion — NOT answered, but now quantified, and it is the most consequential question we have.** It is **half the rows and 72% of all counts**, with a *higher* average than valid rows, plus an undocumented `null` third state; it is **per-sensor rather than per-slot** and flips over a sensor's lifetime. Excluding it swings any impressions figure ~4×. `(sensor_id, timestamp)` is unique, so at least there is no double-count risk. **This one genuinely needs BlueZoo's rule** — ask what `valid` means, what `null` means, and what makes a sensor flip. **Amended 2026-07-27:** much of it is answerable from data we already have, and the `bluezoo-semantics-resolution` workstream will try before asking — `sensor_pulses` carries `expected_pulse_count`/`pulse_count` (a health ratio testable against invalid windows), `group_sensor_history` reveals whether BlueZoo's own group products already exclude invalid sensors, and a per-sensor min/max of `valid` distinguishes a one-way commissioning flag from health-driven flapping (and shows whether `null` is just pre-instrumentation data). What stays BlueZoo's: whether they *intend* consumers to exclude those rows. The ask becomes "we observe X, we plan rule E, confirm?" rather than "what does this mean?"
> - **UTC vs sensor-local — STILL OPEN, and sharper.** `time_zone` turns out to be *sparsely populated* (mostly null); `time_offset` is better populated but DST-varying and also nullable. The tenant spans America/New_York, America/Los_Angeles and offsets `-08:00` … `+03:00`, so a multi-region consumer cannot assume one zone and cannot reliably read one from the row. Ask which column to trust and how daily buckets are cut.
> - **New ask — a docs bug to report to them:** the mandatory time constraint is undocumented, and BlueZoo's own published example (`select * from sensor_visitors limit 1`) fails against the live API.

> **Amended (workstream bluezoo-semantics-resolution, 2026-07-27): the two bullets above are now RESOLVED EMPIRICALLY — each reduced to a single confirmation.** Full evidence: `working-docs/bluezoo-semantics-resolution/findings.md`.
>
> - **`valid` — meaning established, rule recommended.** It is a **one-way commissioning-acceptance flag**: 23 sensors flipped `false`→`true` (zero interleaving, median 21 days in `false` first); currently-valid sensors hold ~0.99 `pulse_count/expected_pulse_count` health vs median 0.00 for never-accepted ones (current-fleet cross-section); it does **not** flip back when a commissioned sensor later degrades (3 live counter-examples); `null` = "before the column existed" (razor cutover 2021-10-14T13:00Z), not a third state; BlueZoo's own groups do **not** exclude invalid sensors (all 19 live invalid sensors are in current group revisions). **Recommended rule R (for 11b): count impressions only from `valid IS TRUE` rows (`WHERE valid`), exclusion logged.** Go-forward impact is ~1.6× (keeps 62% of current-fleet counts) — the historical 4× was inflated by a fleet retired in late 2022. What stays BlueZoo's: confirm the intended consumer behavior, and whether `group_*` aggregates already filter internally — asked as "we observe X, we plan rule R, confirm?" in the outreach draft.
> - **Day-cut, the testable half — ANSWERED: `timestamp` is UTC.** Diurnal troughs of sensors in different zones (−04:00 vs −07:00, May 2024) sit at local midnight *converted to UTC*, displaced by exactly each sensor's own offset. Our local-day bucketing must apply `time_offset` per row (DST-varying) or `time_zone` — which is **not** "sparse" but *recently introduced*: fully populated in current data, null through 2024. What stays BlueZoo's: how *their* `group_*_daily.date` buckets are cut, and which zone column consumers should trust long-term.
...
> **Amended again (same workstream, 2026-07-27) — a second tenant WITH REAL DATA (Morpheus / MO_92) closes most of what the entry below left open.** **Q12 is now fully answered** and the "give us real data" ask is granted. **Q6 and Q11 are resolved with real contents.** **Q18 is mostly closed** — dwell bin scale (0–100 percentages), `distribution_weight` (0 ⇒ null scalars), and row caps (there are none; it is a monthly bytes-scanned quota) are all answered. **Q17's fallback (b) got weaker**: the per-minute table is entitled but dormant since 2025.
>
> Three things genuinely remain BlueZoo's to answer, and the first two are new in character rather than leftovers: **(1) the `valid=false` exclusion rule** — half the rows, 72% of counts, a ~4× swing on any impressions figure, now the single highest-value open question; **(2) the quota's remaining unknowns** — its size is now known (500 GB/sensor-location, raised on request, abuse guard not billing), but *not* how "per sensor location" is charged when one query spans many locations, whether the raise persists past "this month," whether consumption can be checked, or whether the tables are partitioned; **(3) UTC vs sensor-local day cutting**, unchanged but sharper now that `time_zone` is known to be sparsely populated across a multi-timezone tenant. Q1 and Q2 remain decisions, not unknowns — REST is now proven end-to-end against real data.
>
