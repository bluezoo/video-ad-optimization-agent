...
>
> **Fully resolved against real data (same workstream, 2026-07-27).** The column is not vestigial and we now know what it holds: **9,480 of 10,075 rows (94%) carry both `campaign_id` and `campaign_name`**, across 13 campaigns, effectively **1:1 with `group_id`** (campaign 1303 "Hotel Downtown" ↔ group 2196 `hotel-downtown`) and carrying `target_uv` plus `actual_accuracy`. It is a **BlueZoo unique-visitor *measurement* campaign over a sensor group — not an advertising campaign.** That confirms the caveat below rather than undermining it, and makes the `ad_campaign_id` rename unambiguous. Two practical notes for the port: campaign names are free text and need trimming (observed `"Lobbies "`), and `cuv` is a FLOAT extrapolation (82.81, 959.46), never an integer count. Worth one confirmation from BlueZoo that this column will never carry an advertiser's campaign id.
>
...
> **Amended again (same workstream, 2026-07-27) — a second tenant WITH REAL DATA (Morpheus / MO_92, "Hotels International") closes most of what the entry below left open.** **Q12 is now fully answered** and the "give us real data" ask is granted. **Q6 and Q11 are resolved with real contents.** **Q18 is mostly closed** — dwell bin scale (0–100 percentages), `distribution_weight` (0 ⇒ null scalars), and row caps (there are none; it is a monthly bytes-scanned quota) are all answered. **Q17's fallback (b) got weaker**: the per-minute table is entitled but dormant since 2025.
>
> Three things genuinely remain BlueZoo's to answer, and the first two are new in character rather than leftovers: **(1) the `valid=false` exclusion rule** — half the rows, 72% of counts, a ~4× swing on any impressions figure, now the single highest-value open question; **(2) the quota** — its true size, how "per sensor" is charged, whether consumption can be checked, and whether the tables are partitioned, because this constrains Phase 11b's whole access pattern; **(3) UTC vs sensor-local day cutting**, unchanged but sharper now that `time_zone` is known to be sparsely populated across a multi-timezone tenant. Q1 and Q2 remain decisions, not unknowns — REST is now proven end-to-end against real data.
>
