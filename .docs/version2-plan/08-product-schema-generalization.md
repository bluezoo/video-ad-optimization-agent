...
- `app/tools/video_tools.py:476` — consumes that raw dict directly; this call site needs to move to the typed model once it exists.
...
- `app/tools/campaign_tools.py:64` (`create_campaign()`) — hardcodes a `category_mapping` dict that maps fashion product categories to campaign categories, and **silently defaults any unrecognized category to `"essentials"`** rather than erroring (see Phase 1, item 5 — this is dormant drift today, but it's exactly what a non-fashion product's category will hit).
...
- [ ] `app/tools/video_tools.py:476` correctly consumes the typed `Product` model, including its parsed `attributes`, for at least one existing fashion product (proves the adapter migration didn't break the primary video pipeline).
