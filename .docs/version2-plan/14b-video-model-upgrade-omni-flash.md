# Phase 14b — Video Model Upgrade (Gemini Omni Flash / Interactions API, Experimental)
...
- **The `google-genai>=1.55.0` floor in `app/requirements.txt:19` was not verified to include the Interactions surface at all** — the first draft of this phase asserted SDK version 2.11.0 as "confirmed current," but this checkout has no installed environment or lockfile to reproduce that claim against. Treat the minimum working SDK version as unverified until a real spike confirms it.

  > **Amended (workstream 01, 2026-07-14):** partially resolved by a bounded probe during Phase 1. Confirmed with `google-genai` 2.11.0 (installed): `client.interactions` exists (create/get present), so the SDK floor question is answered for ≥2.11.0. Also confirmed: `gemini-omni-flash-preview` returns **404 on the `generate_videos` surface** (Vertex, global) — it is definitively not a drop-in `VIDEO_GEN_MODEL` value. However, naive `interactions.create(request={"body": {...}})` calls fail with `400: value 'UNKNOWN' is not supported for 'type'` for every content part tried (both `text` and `image`, field names matching the SDK's own `ImageContentParam`/text TypedDicts) — the raw-dict body is not serialized the way the wire expects. Step 3's real prototype must use the SDK's typed request path (or capture a working wire example) rather than hand-built dicts; the error's supported-type list is preserved here as a hint: text, image, video, document, content, user_input, model_output, function_call/result, etc.
...
4. Extend the `VIDEO_MODEL_BACKEND` config toggle (parallel to Phase 14a) to support `"veo"` (default) and `"omni_flash"` (experimental, opt-in). Implement the Omni Flash path per the prototype's findings from step 3, normalizing its output into the shared `GeneratedVideo` contract from step 2.
...
Phase 1 (GA Veo baseline in place as the safety net). Not blocked on Phase 14a, though doing 14a first is convenient since Google pairs Omni Flash with Nano Banana 2 Lite in its own materials.
