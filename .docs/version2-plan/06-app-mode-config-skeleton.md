...
> **Amended (workstream replan-data-track, 2026-07-16):** two clarifications after the sibling-repo reassessment. (1) **`APP_MODE` is the only user-facing mode knob.** The donor repo (`ad-campaign-agent`) selects data backends via its own `AUDIENCE_PROVIDER` env var; when Phase 10a ports that provider seam, `APP_MODE` *resolves internally* to a provider selection (`demo` → the synthetic/in-memory provider, `connected` → the live provider) — we do not adopt a second user-facing env var fighting over the same decision. (2) The step-4 protection of `populate_mock_data()`'s unconditional import-time call is **per-phase scoping, not a permanent invariant**: Phase 14 (`15-product-onboarding.md`) deliberately replaces it with explicit, idempotent, gated seeding so a from-scratch (empty-catalog) start is possible.
